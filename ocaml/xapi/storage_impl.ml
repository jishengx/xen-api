...
module Wrapper = functor(Impl: Server_impl) -> struct
  ...
  module VDI = struct
    ...
    let attach context ~dbg ~dp ~sr ~vdi ~read_write =
      ...
      let attach_infos =
        Xapi_stdext_std.Listext.List.filter_map
          (function
            | XenDisk xendisk ->
              Some {
                params = xendisk.Storage_interface.params;
                xenstore_data = [ "backend-kind", xendisk.Storage_interface.backend_type ];
                o_direct = true;
                o_direct_reason = "";
              }
            | _ -> None
      ...
      in
      match attach_infos with
      | attach_info :: _ -> attach_info
      | [] -> raise (Storage_interface.Backend_error (Api_errors.internal_error, ["No XenDisk implementation in Datapath.attach response: " ^ (rpc_of_backend backend |> Jsonrpc.to_string)]))
